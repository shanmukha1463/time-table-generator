...
public class admin_password_change extends JFrame {
	...
	public static admin_password_change frame = new admin_password_change();
	...
	public admin_password_change(String username) {
		uname=username;
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 450, 311);
		contentPane = new JPanel();
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(password.equals(passwordFieldpp.getText())){
					if(passwordFieldnp.getText().equals(passwordFieldrnp.getText())){
						try{
							String query="update admins set password=? where a_id=?";
							PreparedStatement pst=conn.prepareStatement(query);
