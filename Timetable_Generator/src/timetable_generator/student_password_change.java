...
public class student_password_change extends JFrame {

	private JPanel contentPane;

	public static student_password_change frame = new student_password_change();
	 ...
	 * Create the frame.
	 */
	public student_password_change() {
	...
	public student_password_change(String username) {
		uname=username;
		setDefaultCloseOperation(JFrame.EXIT_ON_CLOSE);
		setBounds(100, 100, 450, 311);
		contentPane = new JPanel();
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(password.equals(passwordFieldpp.getText())){
					if(passwordFieldnp.getText().equals(passwordFieldrnp.getText())){
						try{
							String query="update students set password=? where sroll=?";
							PreparedStatement pst=conn.prepareStatement(query);
